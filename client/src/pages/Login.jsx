...
export default function Login() {
  ...
  const oauthErrors = {
    ...
    google_no_user: t('auth.googleNoUser'),
    apple_not_configured: t('auth.appleNotConfigured'),
    apple_failed: t('auth.appleFailed'),
    apple_no_user: t('auth.appleNoUser'),
  };
  const [searchParams] = useSearchParams();
  const oauthError = searchParams.get('error');
  const [email, setEmail] = useState('');
  const [password, setPassword] = useState('');
  const [error, setError] = useState(oauthError ? oauthErrors[oauthError] || t('auth.signInFailed') : '');
    ...
    <div className="auth-page">
      <form className="auth-form" onSubmit={handleSubmit}>
        ...
        <div className="oauth-buttons">
          <a href={`${API_URL}/api/auth/google`} className="btn-oauth btn-google">
          ...
          <span className="btn-oauth btn-apple btn-apple-disabled" title={t('auth.appleTooltip')}>
